    ...
    return (
        <div className={cn('relative', className)}>
            ...
            <span className="z-10 leading-snug font-normal absolute text-center text-[#B7B7B7] absolute bg-transparent rounded text-base items-center justify-center top-1/2 transform -translate-y-1/2 transition duration-300 ease-in-out">
                <SvgImage svg={prefixImg} color={isFocus || isFilled ? '#161616' : '#B7B7B7'} className='transition duration-300 ease-in-out' />
            </span>
                <input
                    ref={ref} {...props}
                    type={inputType}
                ...
                />
                {type === 'password'  && (
                    <button type='button' onClick={() => setShowPassword(prev => !prev)} className="z-10 leading-snug font-normal absolute text-center text-[#B7B7B7] absolute bg-transparent rounded text-base items-center justify-center bottom-[10px] right-0">
                        <SvgImage svg='showPass' color={isFocus || isFilled ? '#161616' : '#B7B7B7'} className='transition duration-300 ease-in-out' />
                    </button>
                )}
            </div>
            {error && (
